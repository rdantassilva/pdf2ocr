...
from pdf2ocr.converters import (convert_docx_to_epub, save_as_docx,
                                save_as_html, save_as_pdf)
from pdf2ocr.logging_config import close_logging, log_message, setup_logging
from pdf2ocr.ocr import process_pdf_with_ocr, validate_tesseract_language
        ...
        with timing_context(f"Processing {filename}", logger) as get_file_time:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                log_messages.append(
                    (
                        "INFO",
                        f"  Layout-preserving PDF created in {get_merge_time.duration:.2f} seconds",
                    )
                )
